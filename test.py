...
def predict_depth(model, img_path, device, img_size=(384,384)):
    ...
    depth_vis = pred / np.max(pred)
    depth_vis = (depth_vis * 255).astype(np.uint8)
    depth_colored = cv2.applyColorMap(depth_vis, cv2.COLORMAP_INFERNO)

    return pred, depth_colored
...
if __name__ == "__main__":
    ...
    for img_path in test_images:
        ...
        depth_map, depth_color = predict_depth(model, img_path, device)

        # Save depth maps
        out_path = os.path.join(output_dir, Path(img_path).stem + "_depth.png")
